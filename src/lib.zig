...
fn serializeType(comptime T: type, buffer: []u8, value: T) usize {
    ...
    inline for (info.@"struct".fields) |field| {
        const size = serializeType(
            field.type,
            buffer[offset..][0..unpaddedSize(field.type)],
...
pub fn collateEntries(allocator: Allocator, entries: []Definition) !std.StringHashMapUnmanaged([]u8) {
